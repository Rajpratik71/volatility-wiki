...
- [Example usage cases and output for Volatility 2.2 commands](Command-Reference22#example-usage-cases-and-output-for-volatility-22-commands)
- [Image Identification](Command-Reference22#image-identification)
	- [imageinfo](Command-Reference22#imageinfo)
	- [kdbgscan](Command-Reference22#kdbgscan)
	- [kpcrscan](Command-Reference22#kpcrscan)
- [Processes and DLLs](Command-Reference22#processes-and-dlls)
	- [pslist](Command-Reference22#pslist)
	- [pstree](Command-Reference22#pstree)
	- [psscan](Command-Reference22#psscan)
	- [psdispscan](Command-Reference22#psdispscan)
	- [dlllist](Command-Reference22#dlllist)
	- [dlldump](Command-Reference22#dlldump)
	- [handles](Command-Reference22#handles)
	- [getsids](Command-Reference22#getsids)
	- [cmdscan](Command-Reference22#cmdscan)
	- [consoles](Command-Reference22#consoles)
	- [envars](Command-Reference22#envars)
	- [verinfo](Command-Reference22#verinfo)
	- [enumfunc](Command-Reference22#enumfunc)
- [Process Memory](Command-Reference22#process-memory)
	- [memmap](Command-Reference22#memmap)
	- [memdump](Command-Reference22#memdump)
	- [procmemdump](Command-Reference22#procmemdump)
	- [procexedump](Command-Reference22#procexedump)
	- [vadinfo](Command-Reference22#vadinfo)
	- [vadwalk](Command-Reference22#vadwalk)
	- [vadtree](Command-Reference22#vadtree)
	- [vaddump](Command-Reference22#vaddump)
	- [evtlogs](Command-Reference22#evtlogs)
- [Kernel Memory and Objects](Command-Reference22#kernel-memory-and-objects)
	- [modules](Command-Reference22#modules)
	- [modscan](Command-Reference22#modscan)
	- [moddump](Command-Reference22#moddump)
	- [ssdt](Command-Reference22#ssdt)
	- [driverscan](Command-Reference22#driverscan)
	- [filescan](Command-Reference22#filescan)
	- [mutantscan](Command-Reference22#mutantscan)
	- [symlinkscan](Command-Reference22#symlinkscan)
	- [thrdscan](Command-Reference22#thrdscan)
- [Networking](Command-Reference22#networking)
	- [connections](Command-Reference22#connections)
	- [connscan](Command-Reference22#connscan)
	- [sockets](Command-Reference22#sockets)
	- [sockscan](Command-Reference22#sockscan)
	- [netscan](Command-Reference22#netscan)
- [Registry](Command-Reference22#registry)
	- [hivescan](Command-Reference22#hivescan)
	- [hivelist](Command-Reference22#hivelist)
	- [printkey](Command-Reference22#printkey)
	- [hivedump](Command-Reference22#hivedump)
	- [hashdump](Command-Reference22#hashdump)
	- [lsadump](Command-Reference22#lsadump)
	- [userassist](Command-Reference22#userassist)
	- [shellbags](Command-Reference22#shellbags)
	- [shimcache](Command-Reference22#shimcache)
	- [getservicesids](Command-Reference22#getservicesids)
- [Crash Dumps, Hibernation, and Conversion](Command-Reference22#crash-dumps-hibernation-and-conversion)
	- [crashinfo](Command-Reference22#crashinfo)
	- [hibinfo](Command-Reference22#hibinfo)
	- [imagecopy](Command-Reference22#imagecopy)
	- [raw2dmp](Command-Reference22#raw2dmp)
- [Miscellaneous](Command-Reference22#miscellaneous)
	- [strings](Command-Reference22#strings)
	- [volshell](Command-Reference22#volshell)
	- [bioskbd](Command-Reference22#bioskbd)
	- [patcher](Command-Reference22#patcher)
	- [pagecheck](Command-Reference22#pagecheck)

# Example usage cases and output for Volatility 2.2 commands
...
The imageinfo output tells you the suggested profile that you should pass as the parameter to --profile=PROFILE when using other plugins. There may be more than one profile suggestion if profiles are closely related. It also prints the address of the KDBG (short for `_KDDEBUGGER_DATA64`) structure that will be used by plugins like [pslist](Command-Reference22#pslist) and [modlist](Command-Reference22#modlist) to find the process and module list heads, respectively. In some cases, especially larger memory samples, there may be multiple KDBG structures. Similarly, if there are multiple processors, you'll see the KPCR address and CPU number for each one. 
...
If the KdVersionBlock is not null, then it may be possible to find the machine's KDBG address via the KPCR. In fact, the backup method of finding KDBG used by plugins such as [pslist](Command-Reference22#pslist) is to leverage kpcrscan and then call the KPCR.get_kdbg() API function. 
...
This plugin does not detect hidden or unlinked processes (but [psscan](Command-Reference22#psscan) can do that).
...
Similar to [cmdscan](Command-Reference22#cmdscan) the consoles plugin finds commands that attackers typed into cmd.exe or executed via backdoors. However, instead of scanning for COMMAND_HISTORY, this plugin scans for CONSOLE_INFORMATION. The major advantage to this plugin is it not only prints the commands attackers typed, but it collects the entire screen buffer (input **and** output). For instance, instead of just seeing "dir", you'll see exactly what the attacker saw, including all files and directories listed by the "dir" command. 
...
To display a process's environment variables, use the envars plugin. Typically this will show the number of CPUs installed and the hardware architecture (though the [kdbgscan](Command-Reference22#kdbgscan) output is a much more reliable source), the process's current directory, temporary directory, session name, computer name, user name, and various other interesting artifacts. 
...
As of 2.1, the new column DumpFileOffset helps you correlate the output of memmap with the dump file produced by the [memdump](Command-Reference22#memdump) plugin. For example, according to the output below, the page at virtual address 0x0000000000058000 in the System process's memory can be found at offset 0x00000000162ed000 of the win7_trial_64bit.raw file. After using [memdump](Command-Reference22#memdump) to extract the addressable memory of the System process to an individual file, you can find this page at offset 0x8000. 
...
To extract all memory resident pages in a process (see [memmap](Command-Reference22#memmap) for details) into an individual file, use the memdump command. Supply the output directory with -D or --dump-dir=DIR. 
...
To conclude the demonstration we began in the [memmap](Command-Reference22#memmap) discussion, we should now be able to make an assertion regarding the relationship of the mapped and extracted pages: 
...
To extract the range of pages described by a VAD node, use the vaddump command. This is similar to [memdump](Command-Reference22#memdump), except the pages belonging to each VAD node are placed in separate files (named according to the starting and ending addresses) instead of one large conglomerate file. If any pages in the range are not memory resident, they're padded with 0's using the address space's zread() method. 
...
To view the list of kernel drivers loaded on the system, use the modules command. This walks the doubly-linked list of LDR_DATA_TABLE_ENTRY structures pointed to by PsLoadedModuleList. Similar to the [pslist](Command-Reference22#pslist) command, this relies on finding the KDBG structure. In rare cases, you may need to use [kdbgscan](Command-Reference22#kdbgscan) to find the most appropriate KDBG structure address and then supply it to this plugin like --kdbg=ADDRESS. 

It cannot find hidden/unlinked kernel drivers, however [modscan](Command-Reference22#modscan) serves that purpose. Also, since this plugin uses list walking techniques, you typically can assume that the order the modules are displayed in the output is the order they were loaded on the system. For example, below, ntoskrnl.exe was first to load, followed by hal.dll, etc. 
...
To extract a kernel driver to a file, use the moddump command. Supply the output directory with -D or --dump-dir=DIR. Without any additional parameters, all drivers identified by [modlist](Command-Reference22#modlist) will be dumped. If you want a specific driver, supply a regular expression of the driver's name with --regex=REGEX or the module's base address with --base=BASE. 
...
- For x86 systems, Volatility scans for ETHREAD objects (see the [thrdscan](Command-Reference22#thrdscan) command) and gathers all unique ETHREAD.Tcb.ServiceTable pointers. This method is more robust and complete, because it can detect when rootkits make copies of the existing SSDTs and assign them to particular threads. Also see the [threads](Command-Reference22#threads) command.
...
To find ETHREAD objects in physical memory with pool tag scanning, use the thrdscan command. Since an ETHREAD contains fields that identify its parent process, you can use this technique to find hidden processes. One such use case is documented in the [psxview](Command-ReferenceMal22#psxview) command. Also, for verbose details, try the [threads](Command-Reference22#threads) plugin. 
...
This plugin isn't generally useful by itself. Its meant to be inherited by other plugins (such as [hivelist](Command-Reference22#hivelist) below) that build on and interpret the information found in CMHIVEs. 
...
If you want to limit your search to a specific hive, printkey also accepts a virtual address to the hive. For example, to see the contents of HKEY_LOCAL_MACHINE, use the command below. Note: the offset is taken from the previous [hivelist](Command-Reference22#hivelist) output. 
...
This plugin parses and prints [Shellbag (pdf)](http://www.dfrws.org/2009/proceedings/p69-zhu.pdf) information obtained from the registry.  For more information see [Shellbags in Memory, SetRegTime, and TrueCrypt Volumes](http://volatility-labs.blogspot.com/2012/09/movp-32-shellbags-in-memory-setregtime.html).
...
The profile should be specified for this command, so if you don't know it already, use the [or [Command-Reference22#kdbgscan kdbgscan](Command-Reference22#imageinfo]) commands first.  The output file is specified with the -O flag.  The progress is updated as the file is converted:
...
By default `strings` will only provide output for processes found by walking the doubly linked list pointed to by PsActiveProcessHead (see [pslist](Command-Reference22#pslist)) in addition to kernel addresses.  `strings` can also provide output for hidden processes (see [psscan](Command-Reference22#psscan)) by using the (capital) -S switch:
