...
Another way to visualize the AS stacking is to break into a [volshell](Command-Reference) and inspect the layering. For example:
    ...
    To get help, type 'hh()'
    >>> addrspace()
    <volatility.plugins.addrspaces.intel.IA32PagedMemoryPae object at 0x100798950>
    >>> addrspace().base
...
In this example, you can see how address spaces stack. We'll use a virtualbox image for the test. Notice how you can reference ".base" to reach the base address space and traverse down the stack. 
    ...
    Use proc() to get the current _EPROCESS object
